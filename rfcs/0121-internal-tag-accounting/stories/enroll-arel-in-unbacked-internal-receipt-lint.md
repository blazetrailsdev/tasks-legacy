...
this story enrolls `arel`.

The work: build the manifest (`pnpm parity:api` then
`pnpm rails-privates:manifest`), add `"packages/arel/src/**/*.ts"` to both
...
- `"packages/arel/src/**/*.ts"` added to the `unbacked-internal-needs-receipt`
  block in BOTH `eslint.config.mjs` and `eslint/rails-private-jsdoc.config.mjs`.
- `pnpm exec eslint --no-inline-config -c eslint/rails-private-jsdoc.config.mjs "packages/arel/src/**/*.ts"`
