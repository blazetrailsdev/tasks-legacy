---
title: "enroll-actionpack-in-unbacked-internal-receipt-lint"
status: ready
updated: 2026-08-25
...
this story enrolls `actionpack`.

The work: build the manifest (`pnpm parity:api` then
`pnpm rails-privates:manifest`), add `"packages/actionpack/src/**/*.ts"` to both
...
- `"packages/actionpack/src/**/*.ts"` added to the `unbacked-internal-needs-receipt`
  block in BOTH `eslint.config.mjs` and `eslint/rails-private-jsdoc.config.mjs`.
- `pnpm exec eslint --no-inline-config -c eslint/rails-private-jsdoc.config.mjs "packages/actionpack/src/**/*.ts"`
