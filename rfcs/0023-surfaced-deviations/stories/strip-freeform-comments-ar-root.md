...
priority: null
pr: 6955
claim: "2026-08-23T22:00:47Z"
assignee: "strip-freeform-comments-ar-root"
blocked-by: null
closed-reason: null
---
...
**This slice: `packages/activerecord/src/*.ts`** — measured ~3714 comment lines / 1749 blocks.
Larger than one PR: split it into several slices of its own (file the next one as you go rather than widening this story).
...
- [ ] `packages/activerecord/src/*.ts` added to the `no-freeform-comments` block's `files` in
      `eslint.config.mjs`.
