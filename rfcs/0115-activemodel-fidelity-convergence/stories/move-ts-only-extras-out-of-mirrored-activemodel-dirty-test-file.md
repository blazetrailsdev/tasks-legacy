---
title: "Move the TS-only extras out of activemodel's mirrored dirty test file"
status: in-progress
updated: 2026-08-24
...
Rails counterpart: `vendor/rails/activemodel/test/cases/dirty_test.rb`.
trails file: `packages/activemodel/src/dirty.test.ts`.

Converged shape: the mirrored `.test.ts` contains only Rails test names; every
TS-only `it(...)` lives in a `dirty.trails.test.ts` sibling under a descriptive
...
- Every `it(...)` `parity:test` reports as "extra (TS only)" for
  `dirty_test.rb` moves to `dirty.trails.test.ts`.
