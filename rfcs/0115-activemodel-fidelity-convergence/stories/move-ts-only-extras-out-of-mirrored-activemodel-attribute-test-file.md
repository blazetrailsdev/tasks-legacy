...
| ------------------- | --- | --------------- |
| `attribute_test.rb` | 33  | 22              |

Rails counterpart: `vendor/rails/activemodel/test/cases/attribute_test.rb`.
trails file: `packages/activemodel/src/attribute.test.ts`.
...
lives in `packages/activemodel/src/attribute.trails.test.ts` under a descriptive
`describe`.

## Acceptance criteria

- Every `it(...)` `parity:test` reports as "extra (TS only)" for `attribute_test.rb`
...
- Afterwards the file reports Extra=0 with OK unchanged at 33, and activemodel's
