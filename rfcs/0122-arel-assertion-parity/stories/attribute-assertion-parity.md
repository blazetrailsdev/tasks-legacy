...
RFC 0122's per-file burndown. Rails file(s): `attributes/attribute_test.rb`
(under `vendor/rails/activerecord/test/cases/arel/`). trails file(s): `packages/arel/src/attributes/attribute.test.ts`.

Measured with `map-minitest-spec-assertion-forms` applied
(`pnpm parity:test -- --assertions --missing --package arel`): **55 assertion-kind
mismatches** remain in this cluster, plus their share of arel's 178
assertion-count and 79 assertion-value mismatches.

Rails asserts complete statements (`SELECT "users"."id" FROM "users" WHERE "users"."id" != 10`) where the port asserts `SELECT * FROM …` — a different projection, so these are value divergences as well as kind ones.
...
- The touched tests pass: `pnpm vitest run packages/arel/src/attributes/attribute.test.ts`.
