...
RFC 0122's per-file burndown. Rails file(s): `visitors/dot_test.rb`
(under `vendor/rails/activerecord/test/cases/arel/`). trails file(s): `packages/arel/src/visitors/dot.test.ts`.

Measured with `map-minitest-spec-assertion-forms` applied
(`pnpm parity:test -- --assertions --missing --package arel`): **16 assertion-kind
mismatches** remain in this cluster, plus their share of arel's 178
assertion-count and 79 assertion-value mismatches.

This file is the `assert_edge` cluster: 50 uses of the file-local helper at `visitors/dot_test.rb:13-15`, which is an `assert_match` over a `->…label="name"` regex. Once `map-minitest-spec-assertion-forms` maps it to `match`, the port's `toContain` (`includes`) is the remaining divergence — the faithful port is `toMatch` on the same regex shape.
