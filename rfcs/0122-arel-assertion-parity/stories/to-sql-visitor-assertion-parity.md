...
RFC 0122's per-file burndown. Rails file(s): `visitors/to_sql_test.rb`
(under `vendor/rails/activerecord/test/cases/arel/`). trails file(s): `packages/arel/src/visitors/to-sql.test.ts`.

Measured with `map-minitest-spec-assertion-forms` applied
(`pnpm parity:test -- --assertions --missing --package arel`): **76 assertion-kind
mismatches** remain in this cluster, plus their share of arel's 178
assertion-count and 79 assertion-value mismatches.

This is also the largest concentration of arel's real value mismatches — `should visit_Float` asserts `1.5` where Rails asserts `"products"."price" = 2.14`, `should visit_DateTime`, `should visit_As` and `should handle Multiplication/Division/Addition/Subtraction` each assert against a differently-constructed node than the Ruby.
...
- The touched tests pass: `pnpm vitest run packages/arel/src/visitors/to-sql.test.ts`.
