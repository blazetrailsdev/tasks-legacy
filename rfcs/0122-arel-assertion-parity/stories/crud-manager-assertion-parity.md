...
RFC 0122's per-file burndown. Rails file(s): `insert_manager_test.rb, update_manager_test.rb, delete_manager_test.rb, crud_test.rb`
(under `vendor/rails/activerecord/test/cases/arel/`). trails file(s): `packages/arel/src/insert-manager.test.ts, packages/arel/src/update-manager.test.ts, packages/arel/src/delete-manager.test.ts, packages/arel/src/crud.test.ts`.

Measured with `map-minitest-spec-assertion-forms` applied
(`pnpm parity:test -- --assertions --missing --package arel`): **24 assertion-kind
mismatches** remain in this cluster, plus their share of arel's 178
assertion-count and 79 assertion-value mismatches.

The three CRUD managers plus crud_test; same `must_be_like` class over INSERT/UPDATE/DELETE statements.
